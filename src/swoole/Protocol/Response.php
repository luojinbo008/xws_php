...
use Swoole\View\Factory as View;
...
class Response
{
    ...
    public static function display($model)
    {
        ...
        $key = \Swoole\Swoole::$php->config['project']['response_time_key'] ?? self::RESPONSE_TIME_KEY;
    ...
    public static function getContent($model)
    {
        ...
        if (is_array($model) && !empty($model['_view_mode'])) {
            ...
            if (empty($viewMode)) {
                $viewMode = \Swoole\Swoole::$php->config['project']['view_mode'] ?? '';
                if (empty($viewMode)) {
                    if (Request::isAjax()) {
                        $viewMode = 'Json';
                    } else {
        ...
        $view = View::getInstance($viewMode);
        if ('Php' === $viewMode) {
